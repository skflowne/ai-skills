...
6. Hand off exactly as drift-review-council's Handoff section specifies (four separate lists: scope reductions, mechanical fixes, patches, refactor tasks with named invariants). If posting to a GitHub PR, always post the review as a normal comment (`COMMENT`), never as a request for changes (`REQUEST_CHANGES`).
