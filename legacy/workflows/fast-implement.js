...
Read and follow the skill's complete process. You own all implementation work: inspect the repository, name the invariants the change must maintain and give each exactly one owning mechanism, define and record cohesive testable milestones that follow from those invariants, choose and implement all appropriate automated coverage (including e2e when warranted), use the persistent independent correctness and test-coverage reviewers, establish RED/GREEN for each milestone, resolve substantive findings, and run final validation. Do not delegate planning, test design, implementation, or validation to a surrounding workflow, and do not post a plan to the issue. Commit per the skill — one commit per milestone and one per fix round, as you go; do not hold the work back for a single commit at the end. Report whether final validation passed, a concise summary, and any failures.`,
