...
| **Scope, seam & state ownership** | Proportionality, root-cause & accretion reviewer | First audit whether the change is the smallest complete, cohesive change at the correct owner — not merely the fewest lines. For every touched file and added dependency, exported API, configuration option, compatibility path, abstraction, or cleanup, trace it to the review intent, a repository constraint, or a necessary migration. Report surface that can be removed while preserving the required behavior, with concrete evidence that no caller or constraint needs it. Do not prefer a local guard or symptom patch over a required wrong-seam refactor just because its diff is smaller. Then review whether the seams are right: report new guards, flags, refs, sequence counters, in-flight counters, or rollback state added to logic that already has several — name the invariant those guards collectively protect and who owns it (one mechanism, or scattered duplicates?). Report hand-rolled async coordination living in UI components instead of an extracted, unit-testable module. Run `git log --name-only` over the touched files and report any recurring across more than two consecutive `fix:` commits. Report components growing past the profile's size limit — or already-oversized components growing at all instead of shrinking. |
