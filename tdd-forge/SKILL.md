---
name: tdd-forge
description: "Implement code changes with one primary author, one persistent independent correctness reviewer, and one persistent automated-test coverage reviewer using milestone-by-milestone RED/GREEN. Use only when the user explicitly names and requests this skill."
---
...
3. Create an explicit plan of cohesive, preferably vertical and independently testable milestones for only the remaining work. Use the plan tool when available.
...
- **Invariants govern useful tests.** Test required user/caller behavior through the real interface at the lowest level that still proves the invariant; test ordering and interleaving against the owning core mechanism. Derive tests from required behavior and realistic regressions, not from the file list. Do not manufacture runtime tests for static types, constants, configuration, or file presence. Every proposed test must meet the canonical test-usefulness standard in [pr-review](../pr-review/SKILL.md).
...
Begin each reviewer prompt with `/skill:pr-review` so both independently apply the canonical scope, isolation, fix-verification, failure-scenario, and test-usefulness contract. The spawn brief is the only time the primary describes the task to a reviewer. Give each reviewer, once: the original task prompt, the requirements, the complete milestone plan, the invariant list with each invariant's owner, the working branch and base ref, and an instruction not to edit files.

Ask the correctness reviewer to check the invariant list for gaps — invariants the change touches but the plan does not name, and any invariant the plan gives more than one owner — and to identify regression risks, validation targets, and missing review gates across the plan. Also give it a standing duplication-and-deletion lens for every milestone: flag code that reimplements an existing shared module or re-inlines an existing constant, and treat any deletion, inlining, or bypass of a shared module or accessor as an escalation — a design question to resolve, never a change to wave through because tests stay green. State the standing expectation for every review: concrete correctness and regression findings ranked by severity with exact file and line references, each carrying a realistic failure scenario — the trigger a real user or caller actually reaches, the mechanism at the cited line, and the real-world impact on the user (what they lose, see wrong, cannot do, or are exposed to), plus how often that state occurs in normal use. Tell both reviewers to drop, not downgrade, any finding whose harm is only "could cause unexpected behavior" or whose trigger the call sites, types, or validation already exclude; a finding without that chain costs a fix round and buys nothing. For findings that are not user-facing — duplication, placement, test structure — the scenario names the realistic edit that will go wrong and the user-visible defect that ships as a result. Do not ask it to co-design the implementation.

Ask the test-coverage reviewer to design automated tests for milestone 1 only, against required user/caller behavior or core logic that milestone owns. For every proposed test, require the real path, independent oracle, realistic implementation regression, expected RED failure, assertions, fixtures/mocks, and command. It must reject tautologies, implementation-derived expectations, mocks that bypass the claimed path, and test-only production hooks. If the milestone has no meaningful runtime behavior to regress, it must say that no automated test is warranted and name the proportional static or deterministic validation instead. Do not request tests for later milestones yet and do not reveal the intended implementation.
...
2. Verify that every proposed test satisfies the canonical test-usefulness standard: it protects required behavior/core logic on a real path, has an independent oracle, and fails under a realistic behavioral regression. Resolve test-design gaps with the reviewer before coding. Drop unwarranted tests rather than inventing runtime behavior or production seams to make them possible. Call `subagent_wait` after each follow-up.
...
10. Request review from both reviewers with only the milestone identifier (e.g. "Review M2") plus the raw, unedited RED and GREEN output (the commands run and their verbatim output) so neither reviewer reruns the same suites. Do not send diffs, change summaries, restated requirements, or areas to focus on: each reviewer finds the milestone's commits and inspects the changes and tests itself. Provide other raw artifacts only when a reviewer asks for them. The correctness reviewer applies its standing brief; the test reviewer checks regression sensitivity, independent oracles, actual user/core paths, false positives, brittle assertions, inadequate RED proof, wrong test level, and production code added solely for the harness. It must not request generic coverage or extra test infrastructure.
11. Call `subagent_wait` for both reviewers. Do not `end_turn` while waiting.
12. Verify findings against original requirements, fix valid ones, rerun RED/GREEN-relevant validation, and commit the fixes. Reply to the appropriate reviewer with only which findings were addressed or rejected (with concrete evidence for rejections), the raw rerun validation output, and a request to re-review the milestone — do not describe the fixes. On re-review, each reviewer inspects the fix range against the pre-fix revision and must not treat its previous finding as an accepted design. It retracts invalid findings and immediately rejects symptom patches, tautological tests, and test-only production mechanisms at the wrong seam. Call `subagent_wait` after each follow-up until both report no substantive remaining findings.
...
- Verify findings against code and original requirements; do not accept them mechanically or let a requested proof expand scope.
- A previous finding is not an accepted design. A fix clears only when its implementation is correct, at the owning seam, and its tests have demonstrated regression-prevention value.
