...
- **Invariants govern warranted tests.** Add or change automated tests only when the task, repository policy, or a realistic regression in changed required behavior/core logic warrants them. Then test a user/caller path through the real interface at the lowest useful level; test ordering/interleaving against the owning mechanism. Do not manufacture runtime tests for static types, constants, configuration, or file presence, and do not add broad coverage merely because a milestone needs validation. Every test must meet the canonical test-usefulness standard in [pr-review](../pr-review/SKILL.md).
...
Begin the reviewer prompt with `/skill:pr-review` so it applies the canonical scope, isolation, fix-verification, failure-scenario, and test-usefulness contract. The spawn brief is the only time the primary describes the task to the reviewer. Give it, once: the original task prompt, the requirements, the complete milestone plan, the invariant list with each invariant's owner, the working branch and base ref, and an instruction not to edit files. State the standing expectation for every review: concrete correctness and regression findings ranked by severity with exact file and line references, each carrying a realistic failure scenario — the trigger a real user or caller actually reaches, the mechanism at the cited line, and the real-world impact on the user (what they lose, see wrong, cannot do, or are exposed to), plus how often that state occurs in normal use. Tell the reviewer to drop, not downgrade, any finding whose harm is only "could cause unexpected behavior" or whose trigger the call sites, types, or validation already exclude; a finding without that chain costs a fix round and buys nothing. For findings that are not user-facing, the scenario names the realistic edit that will go wrong and the user-visible defect that ships as a result.

Ask the reviewer to check the invariant list for gaps — invariants the change touches but the plan does not name, and any invariant the plan gives more than one owner — and to identify regression risks and proportional validation targets across the plan without inventing acceptance criteria or test infrastructure. A reviewer may require credible evidence for required behavior, but it must not prescribe production hooks, environment branches, public APIs, dependencies, or lifecycle changes merely to make a test observe the application. Also give it a standing duplication-and-deletion lens for every milestone: flag code that reimplements an existing shared module or re-inlines an existing constant, and treat any deletion, inlining, or bypass of a shared module or accessor as an escalation — a design question to resolve, never a change to wave through because tests stay green. Do not ask it to co-design the implementation or review automated-test coverage separately.
...
6. Verify findings against the code and original requirements; do not accept them mechanically or let a requested proof expand task scope.
7. Fix valid findings, rerun relevant validation, and commit the fixes. Reply with only which findings were addressed or rejected (with concrete evidence for rejections), the raw rerun validation output, and a request to re-review the milestone — do not describe the fixes. On re-review, require the reviewer to inspect the fix range against the pre-fix revision and original requirement, not merely check whether its comment was followed. It must retract an invalid finding and immediately classify a symptom patch, misplaced test mechanism, or test-only production hook as wrong-seam work. Call `subagent_wait` after each follow-up until no substantive findings remain.
...
- A previous finding is not an accepted design. Re-review every fix for correctness, scope, owning seam, and useful regression evidence; code that only satisfies the comment does not close it.
