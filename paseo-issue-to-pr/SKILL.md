...
Require an issue number. Resolve the repository, authenticated GitHub identity, default or explicitly stacked base, clean-tree state, active Paseo agents, and current `paseo run --help`; verify issue scope and repository instructions rather than asking for facts that can be inspected. Before launching implementation, prove the controller can reach GitHub with `gh auth status`, `gh api user`, and a read of the target issue or repository. Record the authenticated login and never pass credentials through prompts, logs, or `--env`. Leave unrelated dirty work untouched and branch from the selected committed base, recording anything excluded from the run.
