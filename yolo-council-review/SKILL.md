...
2. Follow the canonical [review output contract](../pr-review/SKILL.md#review-output-contract). If no findings survive, return only `No findings.` plus material validation or limitations, then stop the handoff; do not include per-expert verdicts, clean-area summaries, empty plans, rejected candidates, or ask for plan approval.
